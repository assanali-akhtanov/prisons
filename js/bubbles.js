...
let containerWidth = 1230;
let containerHeight = 830;
let diameterRatio = 5000;
let leftRatio = 3000;
let topRatio = 1.25;
...
let bubblesInner = document.getElementById('bubbles-inner');
let bubbleHeader = document.getElementById('bubble-header');

function scaleBubbles() {
  let heightRatio = window.innerHeight/(containerHeight + 150);
  let widthRatio = window.innerWidth/(containerWidth + 150);
...
data.forEach(function(country, i){
  ...
  if (country.name === "USA") {
    ...
    let innerUSA = div.querySelectorAll(".country-bubble-inner");

    let usaRate = document.createElement("div");
    usaRate.id = "usa-bubble-rate";
    usaRate.innerHTML = "Rank per capita: 1";
    innerUSA[0].appendChild(usaRate);

    let usaCount = document.createElement("div");
  ...
  let diameter = Math.floor(country.population / diameterRatio);
...
function recalculateUSA(prisoners) {
  let usa = document.getElementById("country-bubble-usa");
  let rate = (prisoners/328000000) * 100000;

  let usaCount = document.getElementById("usa-bubble-count");
  usaCount.innerHTML = "Rank in absolute: " + getRankPopulation(prisoners);

  let usaRate = document.getElementById("usa-bubble-rate");
  usaRate.innerHTML = "Rank per capita: " + getRankRate(rate);

  let diameter = Math.floor(prisoners / diameterRatio);
...
let usa_prisoners = 2300000;
let checks = document.querySelectorAll('.release');
...
let radios = document.querySelectorAll('.release-radio');
radios.forEach(function(radio){
  radio.addEventListener('change', function(e){
    let change = parseInt(e.target.value);
    let sibiling = document.getElementById(e.target.dataset.sibiling);
